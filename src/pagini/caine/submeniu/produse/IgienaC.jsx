...
export const IgienaC = (props) => {
  const [produse, setProduse] = useState([]);
  const navigate = useNavigate();

  const getProduse = async () => {
    try {
      const querySnapshot = await getDocs(collection(db, 'Caini/Caini/Igiena'));
      const listaProduse = querySnapshot.docs.map((doc) => ({
        ID: doc.id,
        ...doc.data(),
        Cantitate: 1, // Setăm inițial cantitatea la 1
      }));
 ...
 const [produseTotale, setProduseFinale] = useState(0);
  ...
  return (
    <>
      <Link to='/câini'>
        <button className='btn'>Înapoi</button>
      </Link>
      <br />
      {produse.length > 0 && (
                <div className='container-fluid'>
                    <h1 className='text-center-caini'data-heading='Igienă'>Igienă</h1>
