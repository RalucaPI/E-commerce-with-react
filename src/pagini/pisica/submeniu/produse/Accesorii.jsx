...
import './style.css'
export const Accesorii = (props) => {
    const [produse, setProduse]=useState([]);

  const getProduse = async () => {
   try {
    const querySnapshot = await getDocs(collection(db, "Pisici/Pisici/Accesorii"));
 ...
 const [produseTotale, setProduseFinale] = useState(0);
  ...
  const addToCart = async (produs) => {
    ...
    return (
      <>
      <div className='pisici-produse-box'>
          <Link to='/pisici'>
          <button className='btn'>Inapoi</button>
        </Link>
        <br />
        <div className='container-fluid'>
          <h1 className='text-center-pisici'  data-heading='Accesorii'>
            Accesorii
          </h1>
          {produse.length > 0 ? (
